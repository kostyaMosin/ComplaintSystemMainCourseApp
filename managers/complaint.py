...
class ComplaintManager:
    ...
    async def create_complaint(complaint_data, user):
        ...
        os.remove(path)
        async with database.transaction() as t_conn:
            id_ = await t_conn._connection.execute(complaint.insert().values(complaint_data))
            await ComplaintManager.issue_transaction(
                t_conn, complaint_data["amount"], f"{user['first_name']} {user['last_name']}", user["iban"], id_
            )
    ...
    async def approve(id_):
        ...
        complainer = await UserManager.get_user_by_complain_id(id_)
        ses.send_mail(
            "Complaint approved!",
            [complainer["email"]],
    ...
    async def reject(id_):
        ...
        ses.send_mail(
            "Complaint rejected!",
            [complainer["email"]],
            "Your claim is rejected. Call your approver to have more details."
        )
